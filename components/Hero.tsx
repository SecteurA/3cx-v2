...
export default function Hero() {
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success('Thank you for your interest!', { description: 'Our team will contact you shortly.' });
  };
  ...
  return (
    <section className="relative bg-[#0A1428] mt-16">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 min-h-screen flex items-center py-12">
        <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="space-y-8">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-white sm:text-6xl lg:text-5xl xl:text-6xl">
              Transform Your <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary/80 to-accent">Business Communications</span>
            </h1>
            ...
            <div className="flex items-center gap-4">
              ...
              <Button size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white border-0">Learn More</Button>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <Card className="backdrop-blur-sm bg-white/[0.03] border-white/10">
              <CardHeader>
                ...
                <CardDescription className="text-white/70">Fill out the form below for a free consultation</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <Input placeholder="Full Name" required className="bg-white/5 border-white/10 text-white" />
                  <Input type="email" placeholder="Business Email" required className="bg-white/5 border-white/10 text-white" />
                  <Input type="tel" placeholder="Phone Number" required className="bg-white/5 border-white/10 text-white" />
                  <Input placeholder="Company Name" required className="bg-white/5 border-white/10 text-white" />
                  <Input type="number" placeholder="Number of Users" required className="bg-white/5 border-white/10 text-white" />
                  <Button type="submit" className="w-full bg-primary hover:bg-primary/90" size="lg">
                    <PhoneCall className="mr-2 h-4 w-4" /> Schedule Consultation
                  </Button>
                </form>
              </CardContent>
            </Card>
