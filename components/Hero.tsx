...
export default function Hero() {
  ...
  return (
    <section className="relative bg-[#0A1428] pt-16"> {/* Removed extra margin */}
      {/* Background patterns */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#0A1428_1px,transparent_1px),linear-gradient(to_bottom,#1a365d_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_0%,#000_70%,transparent_110%)]"></div>
      ...
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-accent/20 rounded-full filter blur-3xl opacity-20 animate-pulse delay-700"></div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="space-y-8">
            <div className="inline-flex items-center rounded-full border border-primary/20 px-4 py-1.5 text-sm font-medium bg-white/5 backdrop-blur-sm">
              ...
              <div className="mx-2 h-4 w-[1px] bg-primary/20"></div>
            ...
            <div className="flex items-center gap-4">
              <Button size="lg" className="bg-primary hover:bg-primary/90 group" onClick={() => window.location.href = '#services'}>
                View Pricing
                <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </Button>
              <Button size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white border-0" onClick={() => window.location.href = '#features'}>
                Learn More
              </Button>
            </div>
          </motion.div>
